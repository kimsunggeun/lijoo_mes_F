...
import { getToken, setToken, setJWT, getUserId, getComCode } from '@/utils/token'
  ...
  response => {
    ...
    if (!response.data.code) {
      if (response.config.responseType == 'blob') {
        ...
        //   res.success = res.result.success = response.data.success = false
        //   response.data.code = 9999
        //   response.data.msg = response.headers.customheader
        // } else {
          res.success = res.result.success = response.data.success = true
        // }
      }
    }
