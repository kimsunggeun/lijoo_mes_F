    ...
    <v-row no-gutters fill-height>
      ...
      <v-col cols="8" class="pa-1">
        <v-tabs color="#000" @change="tabChange">
          <v-tab class="mr-2">{{$t('tabRouteInfoTitle')}}</v-tab>
          <v-tab class="mr-2">{{$t('tabBomInfoTitle')}}</v-tab>
          <v-tab-item>
            <v-row>
              <i-card-vertical
...
import { getFacCode, getProCodeNameWithPathCode } from '@/api/view/bas0215'
...
export default {
  ...
  methods: {
    ...
    bomInfoEditingStart(e) {
      if (e.data.__created__ === false) {
        if (e.column.dataField === 'proNo' || e.column.dataField === 'proCode' || e.column.dataField === 'level') {
          e.cancel = true
        }
      }
    },
    ...
    btnSearch() {
      ...
      const searchData = {
        comCode: this.searchFacCode == null ? '' : this.searchFacCode.toString(),
    ...
    //경로유형(BOM Tab) Cell클릭
    async pathTypeFocRowchange2(e) {
      if (e.rowIndex >= 0 && e.row && e.row.data) {
        const processGetProCode = new Promise((resolve, reject) => {
          const proCode = {
            comCode: getComCode(),
            facCode: this.searchFacCode,
            pathCode: e.row.data.pathCode
          }
          getProCodeNameWithPathCode(proCode).then(res => {
            if (res.success) {
              this.lookup.proCode = res.list
              resolve('success')
            }
            else {
              resolve('fail')
        ...
        const processResult = await processGetProCode
    ...
    async btnSave(type) {
      ...
      if (type === 'item') {
        ...
        //필수값 체크
        let valCheckData = bomRows.findIndex(e => 
          e.comCode === '' || e.facCode === '' || e.paPartNo === '' || 
          e.pathType === '' || e.pathCode === '' || e.proCode === '' || 
          e.partNo === '')
        if (valCheckData >= 0) {
        ...
        for (var i = 0; i < bomRows.length; ++i) {
          const insertData = {
            comCode: bomRows[i].comCode,
            facCode: bomRows[i].facCode,
            paPartNo: bomRows[i].paPartNo,
            pathType: bomRows[i].pathType,
            pathCode: bomRows[i].pathCode,
            proNo: bomRows[i].proNo,
            proCode: bomRows[i].proCode,
            level: bomRows[i].level,
            dispNo: bomRows[i].dispNo,
            partNo: bomRows[i].partNo,
            amount: bomRows[i].amount,
            remark: bomRows[i].remark,
            userFlag: bomRows[i].userFlag,
            makeDate: bomRows[i].makeDate,
            maker: bomRows[i].maker,
            editDate: null,
            editor: null,

            __created__: bomRows[i].__created__,
            __modified__: bomRows[i].__modified__            
          }
    ...
    itemPopOpen(e) {
      if (e.column.dataField === 'partNo') {
        this.$refs.grdItemData.open(e.rowIndex)
      }
    },

    getItemChoice(...arg) {
      this.gridBomInstance.cellValue(arg[0], 'partNo', arg[1][0].partNo)
    },
