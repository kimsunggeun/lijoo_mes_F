              ...
              <v-row no-gutters class="center pa-2 mr-0">
                ...
                <v-row no-gutters>
                  ...
                  <v-col cols="12" md="2" class="pl-1 mt-2 mr-2">
                    <DxDateBox :value="reqDtFrom" :max="reqDtTo" display-format="yyyy-MM-dd" height="90%"
                  ...
                  <v-col cols="12" md="2" class="pt-2 mr-2">
                    <DxDateBox :value="reqDtTo" :max="reqDtTo" display-format="yyyy-MM-dd" height="90%"
                      @value-changed="e => { reqDtTo = e.value && getDateFormat(e.value) }" />
                  </v-col>
                  <v-col cols="12" md="2" class="pt-2 mr-2">
                  <v-text-field label="품목명" class="search-text-field" dense outlined color="primary" hide-details="auto"
                    :prepend-inner-icon="$t('$search')" @keydown.enter="btnSearch()" clearable v-model="txtMatNm" />
                  </v-col>
                  <v-col cols="12" md="2" class="pt-2 mr-2">
                    <v-autocomplete :menu-props="{ offsetY: true }" color="primary" class="required mx-1 rounded-0" item-text="matNm"
                      item-value="" :prepend-inner-icon="$t('$search')" label="공장" dense outlined>
                    </v-autocomplete>
                  </v-col>
                </v-row>
              </v-row>
...
<v-col cols="12" class="pa-2">
        <i-card-vertical headerTitle="returnReg" :useBtnList="[
          'btnAdd', //신규
          'btnSave', //저장
          'btnDelete' //삭제
            ...
            <v-layout column>
              <v-col cols="12" class="pa-0">
                <DxDataGrid 
                  id="returnRegInfo" 
                  class="listBox03" 
                  :ref="returnRegInfoRef" 
                  :data-source="returnRegInfo"
                  ...
                  @editing-start="sal0023EditingStart"
                  >
                  ...
                  <DxColumn data-field="lotDetail" :caption="$t('lotDetail')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="retLotNo" :caption="$t('retLotNo')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="retLotDetail" :caption="$t('retLotDetail')" width="auto" data-type="string" alignment="center" css-class="devest-grid-header-require" :allow-editing="false" />
                  <DxColumn data-field="retCode" :caption="$t('retCode')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="retType" :caption="$t('retType')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="whCode" :caption="$t('whCode')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="partNo" :caption="$t('partNo')" width="auto" data-type="string" alignment="center" />
                  <DxColumn data-field="retQty" :caption="$t('retQty')" width="auto" data-type="string" alignment="center" />
...
import { getReturnRegInfo } from '@/api/view/sal0023'
...
export default ({
  ...
  data() {
    return {
      returnRegInfo: [],
      returnRegInfoRef: 'returnRegInfoRef',
  ...
  computed: {
    gridMainInstance() {
      return this.GetDataGrid(this.returnRegInfoRef)
    },
  },
  methods: {
    sal0023EditingStart(e){
      if (e.data.__created__ === false) {
        if (e.column.dataField === 'ordNo' || e.column.dataField === 'ilNo' || e.column.dataField === 'no' || e.column.dataField === 'retLotDetail') {
          e.cancel = true
        }
      }
    },
    ...
    doSearchMainList(isProgress) {
      ...
      //   comCode: getComCode(),
      // }
      // getReturnRegInfo(params).then(res => {
      //   this.returnRegInfo = res.list
      // }).finally(() => {
      //   this.endLoading()
      // })
    }, 
    btnAdd() {
      let newRow = {
        id: this.returnRegInfo.length + 1,
        comCode: '',
        facCode: '',
        ordNo: '',
        ilNo: '',
    ...
    async btnSave() {
      ...
      for (var row of selectedMainRows) {
        ...
        if (!row.ordNo || !row.ilNo || !row.no || !row.retLotDetail ) {
          this.vToastify('필수항목 입력하세요.[수주번호, 일련번호, 출하순번, 반품 전산번호]', 'warn')
          return
        }
