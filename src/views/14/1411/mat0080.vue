            ...
            <v-layout column overflow-auto>
              <v-col cols="12" class="pa-0">
                <DxDataGrid
                  ...
                  class="listBox"
                  :ref="uploadPurchaseOrderRef"
                  :data-source="uploadPurchaseOrder"
                  ...
                  :selection="{ mode:'none' }"
                  @cellPrepared="cellPreparedMainGrid"
                >
...
import { getPurchaseOrder, savePurchaseOrder } from '@/api/view/mat0080'
...
export default {
  ...
  data() {
    return {
      ...
      uploadPurchaseOrder: [],
      uploadPurchaseOrderRef: 'uploadPurchaseOrderRef',
  ...
  computed: {
    gridMainInstance() {
      return this.GetDataGrid(this.uploadPurchaseOrderRef)
    },
    gridPopupInstance() {
  ...
  methods: {
    ...
    async btnSave() {
      ...
      this.vToastifyPrompt(
        ...
        () => {
          ...
          for(let i in selectAll._items) {
            if(selectAll._items[i].custCode !== null) {
              this.gridMainInstance.beginUpdate()
            } else {
              this.vToastify('거래처 코드 내용이 없습니다.', 'warn')
              return
            }
            if(selectAll._items[i].partNo !== null){
              this.gridMainInstance.beginUpdate()
            } else {
              this.vToastify('품번 내용이 없습니다.', 'warn')
              return
            }
          }
          this.openLoading()
          savePurchaseOrder(selectAll._items)
            .then(res => {
              if (res.success) {
    ...
    //엑셀 다운로드
    btnExcel() {
      let workSheet = this.$xlsx.utils.table_to_sheet(document.getElementById('uploadPurchaseOrder')) // table id를 넣어주면된다

      const workBook = this.$xlsx.utils.book_new() // 새 시트 생성
      this.$xlsx.utils.book_append_sheet(workBook, workSheet, 'sheet1') // 시트 명명, 데이터 지정
      this.$xlsx.writeFile(workBook, '구매발주.xlsx') // 엑셀파일 만듬
    },
    //------ 업로드 팝업 -------
    //컬럼 색 변경
    cellPrepared(e) {
    ...
    cellPreparedMainGrid(e){
      if(e.columnIndex === 1 && e.data.custCode === null){
        e.cellElement.style.backgroundColor = 'red'
      } else if(e.columnIndex === 3 && e.data.partNo === null) {
        e.cellElement.style.backgroundColor = 'red'
      }
    },
    // custCode or partNo
    ...
    confirmBtn() {
      ...
      this.tmpResult2[0].forEach(e => {
        this.tempData.push({
          ...
          regiDate: e[this.required[0].mapping],
      ...
      getPurchaseOrder(this.tempData).then(res => {
        this.uploadPurchaseOrder = res.list
      })
