  ...
  <v-container fluid align-start ma-0 pa-2 class="container-height">
    <v-row no-gutters class="header pa-3">
      <v-col cols="5">
        ...
        <span class="headerText2">{{$t('unplannedShipment')}}</span>
      </v-col>
      <v-col class="d-flex">
...
import { getPop0420OrderNo, getPop0420UnPlannedShipmentInfo } from '@/api/view/pop0420'
...
export default {
  ...
  methods: {
    ...
    doSearchOrdNo() {
      ...
      getPop0420OrderNo(searchData).then(res => {
    ...
    doSearchOrderStatus() {
      ...
      getPop0420UnPlannedShipmentInfo(searchData).then(res => {
        if (res.success) {
          ...
          for (let i = 0; i < res.list.length; ++i) {
            // res.list[i].orderQty = this.numberWithCommas(res.list[i].orderQty)
            // res.list[i].outQty = this.numberWithCommas(res.list[i].outQty)
            // res.list[i].nowQty = this.numberWithCommas(res.list[i].nowQty)
            // res.list[i].price = this.numberWithCommas(res.list[i].price)
            // res.list[i].amount = this.numberWithCommas(res.list[i].amount)
          }

          this.topGrid = res.list
          //if (this.isEmpty(this.itemGridOrderStatusClickData) === true) {
            this.itemGridOrderStatusClickData = this.topGrid[this.rowIndex]
          //}
        ...
        if (res.success) {
          ...
          for (let i = 0; i < res.list.length; ++i) {
            if (parseInt(res.list[i].stockQty) > 0) {
              //res.list[i].stockQty = this.numberWithCommas(res.list[i].stockQty)
              res.list[i].id = newId++
    ...
    async doRegistShipment() {
      ...
      for (let i = 0; i < selectedRegistRows.length; ++i) {
        totalOutQty += parseInt(selectedRegistRows[i].stockQty)
      }

      if (Number(totalOutQty) > Number(orderNowQty)) {
              console.log("토탈outQty", totalOutQty)
        console.log("오더NowQty", orderNowQty)
      ...
      getSal0120(searchData)
        .then(async res => {
          ...
          for (var i = 0; i < selectedRegistRows.length; ++i) {
            ...
            if (Number(insertData.stockQty) < Number(insertData.outQty)) {
